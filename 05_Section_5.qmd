...
\end{aligned}
$$
where the last line follows from $X \Perp Z$. Then, since $\beta_0+\beta_1 E(X)$ is a constant,
$$
\begin{aligned}
...
Now, given $X, \beta_0+\beta_1 X$ is constant, and this very fact, along with $X \Perp Z$, yields
$$
\begin{aligned}
...
\end{aligned}
$$
where we recall that $E(E(Y \mid X, Z) \mid X)=E(Y \mid X)$ by Proposition 5.7.1. This is precisely the identity of variances that we needed to exhibit, so we are done.s
-->
:::
...
### ✏️ Section Problem 5
3. Let $X$ and $Y$ be random variables with $\operatorname{Var}(Y)<\infty$.
